...
class tcp_client:
    def __init__(self, room_name_soze, operation, state, room_name, password, user_name) -> None:
        self.__socket: socket.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.__server_address: str = "0.0.0.0"
        self.__server_port: int = 9001
        self.__buffer: int = 32
        self.__host_token_buffer: int = 128
        self.__room_name_size: int = room_name_soze
...
def main() -> None:
    client: tcp_client = tcp_client(3, 2, 1, "room1", "password",  "morio")
    client.start()
